...
type FlowToExecutionPlanType = {
	...
	error?: {
		...
		invalidElements?: AppNodeMMissingInputs[];
	};
};
...
export function FlowToExecutionPlan(
	...
	if (!entryPoint) {
		return {
			error: {
				type: FlowToExecutionPlanTypeValidationError.NO_ENTRY_POINT,
			},
		};
	}

	const inputsWithErrors: AppNodeMMissingInputs[] = [];
	...
	if (inputsWithErrors.length > 0) {
		return {
			error: {
				type: FlowToExecutionPlanTypeValidationError.INVALID_INPUTS,
